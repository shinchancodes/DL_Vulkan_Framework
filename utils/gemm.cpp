...
GEMM::GEMM(VulkanContext& ctx)
    : ctx_(ctx) {}

std::vector<float> GEMM::run(const std::vector<float>& A,
                               const std::vector<float>& B,
                               uint32_t M, uint32_t K, uint32_t N)
{
    if (A.size() != (size_t)M * K)
        throw std::invalid_argument("GEMM: A size mismatch");
    if (B.size() != (size_t)K * N)
        throw std::invalid_argument("GEMM: B size mismatch");
